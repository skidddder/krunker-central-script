...
}, true, ["verify"]).then(async _0xe62a41 => {
  async function _0x4f942f(_0x107250) {
    let _0x3e409d;
    try {
      _0x3e409d = JSON.parse(atob(_0x107250));
    } catch {
      console.log("Couldn't verify key: invalid encoding");
      return false;
    }
    if (!(parseInt(_0x3e409d.message) > Date.now())) {
      console.log("Couldn't verify key: bad time");
      return false;
    }
    const _0x525137 = await crypto.subtle.verify({
      name: "RSASSA-PKCS1-v1_5"
    }, _0xe62a41, new Uint8Array(_0x3e409d.signature), new TextEncoder().encode(_0x3e409d.message));
    if (!_0x525137) {
...
fetch(serverUrl + "/current-version").then(_0xbe26ae => _0xbe26ae.text()).then(_0x3c419e => {
  if (_0x3c419e !== version) {
    const _0x2a0d46 = confirm("This version of the cheat is no longer working, we have released a new version of the cheat (" + version + " -> " + _0x3c419e + "),\n\nPress OK to be directed to Krunkercentral.com, once you're there download the hack from the latest post");
    if (_0x2a0d46) {
      ...
      for (var _0x44f204 = 0; _0x44f204 < _0x34b222.addedNodes.length; _0x44f204++) {
        ...
        if (_0x4a655f.tagName === "SCRIPT") {
          if (_0x4a655f.innerHTML.includes("@license Krunker.io")) {
            _0x4a655f.remove();
            const _0x283dcc = downloadFileSync(serverUrl + "/game_1_4.js");
            Function(id + "();\n\n" + _0x283dcc)();
          }
        }
      }
